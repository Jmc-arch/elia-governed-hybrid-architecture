# Operating Mode: Elia - Architecture v0.2

## Project Structure

```
ELIA_ROOT/
├── core/               # CR_* — Critical foundation modules (cannot fall)
│   ├── CR_HUB.py       # Central Communication Node
│   ├── CR_SEC.py       # Centralized Security Module
│   ├── CR_SYN.py       # Synchronization Module
│   ├── CR_GSC.py       # Global Stability Controller
│   └── CR_MEM.py       # Persistent Storage
│
├── logic/              # SV_* — Business logic and filters
│   ├── SV_LOG.py       # Unified Logging System
│   ├── SV_CFG.py       # Dynamic Configuration
│   ├── SV_EVF.py       # Environment Variance Filter
│   ├── SV_ICF.py       # Input Classification & Filtering
│   ├── SV_HVE.py       # Hybrid Verification Engine
│   ├── SV_DLG.py       # Dialogue Management
│   └── SV_NGA.py       # Neural Gating Authority
│
├── neural/             # SV_* — Neural runtime
│   ├── SV_NIE.py       # Neural Inference Engine
│   └── SV_LRN.py       # Targeted Learning
│
├── interaction/        # SV_* — Human ↔ IA boundary
│   └── SV_UIP.py       # User Interaction Point
│
├── gateway/            # EX_* — External connectivity
│   ├── EX_EGC.py       # Egress Connector (Outbound)
│   └── EX_IGC.py       # Ingress Connector (Inbound)
│
└── tests/              # SV_* — Quality & tests (optional, non-runtime)
    └── SV_TSR.py       # Test Suite Runner
```
...
|--------|------|-------------|
| `CR_*` | Critical | Foundation modules — system stops if they fall |
| `SV_*` | Service | Business logic and functional services |
| `EX_*` | External | External connectivity modules (network boundary) |
...
## CR_HUB: Central Communication Node (Pure Routing)
...
**Logging Relay**: Routes all logging events to SV_LOG without persisting them itself.
...
- `get_isolation_status()`: List of modules currently isolated by CR_GSC decision.
...
- CR_HUB detects message delivery failures (timeout, network error, module unavailable).
- Mechanism:
- After 3 consecutive failures to the same module, CR_HUB publishes a `module_unreachable` event to the "system_health" topic.
- CR_GSC, subscribed to this topic, receives the alert and decides on the action (isolation, restart attempt, degraded mode).
- If CR_GSC decides to isolate the module, it publishes an `isolate_module` event containing the module identifier and isolation duration.
- CR_HUB, receiving this order, applies the isolation by temporarily refusing to route messages to this module.
- Default isolation duration: 30 seconds, adjustable by CR_GSC based on severity.
- Restoration:
- Automatic after expiration of the isolation delay
- Or manual via `reset_circuit_breaker(module_id)` command called by CR_GSC

**State Persistence**: Circuit breaker state saved in CR_MEM via CR_SYN every 10 seconds.
- Automatic restoration from CR_MEM on restart, applied after CR_HUB initialization.

**Supervision**: CR_GSC monitors circuit breaker state via get_isolation_status interface.
- Alert SV_LOG if circuit breaker active for more than 5 consecutive minutes.

**Fallback Mode**: If CR_HUB overloaded (>80% CPU for 1min, rare with pure routing):
- Direct point-to-point routing via pre-loaded addresses (temporary broker bypass).
- Alerts via SV_LOG (critical level).
- Return to normal orchestrated by CR_GSC after CR_HUB CPU <60% for 2 minutes, with confirmation by receipt of 3 consecutive heartbeats by the modules.

**Direct Routing Mechanism**:
**Prerequisites at Startup:**
Each module loads a static routing table from SV_CFG at the time of its initialization.
This table contains the network addresses of all modules with which it communicates directly.
Table format (example for SV_ICF):
`direct_routes: { SV_HVE: "tcp://localhost:5001", SV_DLG: "tcp://localhost:5002", SV_LOG: "tcp://localhost:5003" }`
**Fallback Mode Activation:**
Receiving modules detect the absence of CR_HUB signal after 15 seconds (3 consecutive missed heartbeats).
...
Beyond that, CR_GSC triggers a coordinated system restart.
**Return to Normal Mode:**
CR_HUB publishes the "hub_recovered" event after stabilization (CPU < 60% for 2 minutes).
Modules switch to normal mode after receiving this event AND confirmation by receipt of 3 consecutive CR_HUB heartbeats.
**Mutual Supervision Mechanism (Heartbeat)**:
- CR_HUB emits a presence signal every 5 seconds to all modules subscribed to the "system_heartbeat" topic.
- This signal contains the router state, queue size, and system stability metrics when available.
- These stability metrics are provided by CR_GSC via an asynchronous deposit every 3 seconds in a shared space readable by CR_HUB.
...
- In parallel, CR_GSC monitors the health of critical modules (SV_LOG, CR_MEM, CR_SYN) by observing their responses to signals emitted by CR_HUB.
- This monitoring is performed independently of the metrics deposit, avoiding any circular dependency: CR_GSC consumes CR_HUB signals to detect anomalies, but does not condition its own activity on the receipt of these signals.
- In case of prolonged absence detection (3 consecutive missed signals, i.e., 15 seconds), CR_GSC triggers a critical alert and may initiate a restart attempt of the failing module via system signal if the recovery policy allows it.
- Receiving modules detect the absence of CR_HUB signal after 15 seconds (timeout) and automatically switch to direct point-to-point routing mode (direct synchronous calls) to preserve service continuity, while emitting a local alert.
...
- Transmission of critical notifications: CR_HUB queries CR_GSC via `check_system_stability()`.
...
**CR_MEM - Persistent Storage**
- Atomic Read Latency: P95 < 5ms
- Atomic Write Latency: P95 < 10ms
- Availability: 99.99% (critical data)
- Degraded Behavior: Temporary buffer CR_SYN, deferred writes

**SV_NIE - Neural Inference**
...
SLOs are continuously measured by SV_LOG and trigger progressive alerts (warning at 80% threshold, critical at 95% threshold) allowing intervention before complete service breakdown.

---

## CR_SEC: Centralized Security Module
...
- JWT token management for EX_IGC.
...
## CR_SYN: Synchronization Module
**Role**: System state management, cache, backup/recovery, and CR_MEM access.
**Features**:
- System state synchronization: Manages global flags of `BehaviorConfig` and global state (`InteractionMode`).
- Shared storage interface: Provides standardized accessors for CR_MEM with lazy loading. Unique entry point to CR_MEM.
- **Accessibility Check**: Before any operation, CR_SYN checks the physical availability of CR_MEM via test read attempts (maximum 5 attempts spaced at 0.5s, 1s, 2s, 4s, 8s).
- If failure after 15 cumulative seconds, immediate system shutdown with critical error code "STORAGE_UNREACHABLE".
- This check runs during Phase 0 (System Initialization), after CR_MEM launch but before operational modules activation.
...
- Translates complex operations into atomic sequences to CR_MEM.
...
- If after evicting 1000 consecutive entries the space remains insufficient, a critical alert "CACHE_EXHAUSTED" is issued via SV_LOG and new insertions are temporarily refused until natural space liberation (TTL expiration).
...
- State backup: Saves state in CR_MEM every 60s.
...
**Finalization Zone** (managed by CR_SYN):
...
5. Publish via CR_HUB
6. Release Finalization Zone lock

**Anti-Deadlock Guarantees**: CR_MEM has no reference to CR_SYN or other modules.
- CR_SYN holds references to CR_MEM but always releases coordination locks before outgoing calls to CR_HUB or acquisition of the finalization lock.
- Mandatory timeout on locks — coordination: 500ms, finalization: 2 seconds, global fallback: 10 seconds.
- If timeout reached, forced release with critical alert in SV_LOG.
- Cycle detection: CR_SYN maintains a lock acquisition graph in memory. Before each acquisition, check for absence of potential cycle.
...
- If cache miss, brief acquisition of coordination_lock, call CR_MEM.atomic_read, update L1 cache, release coordination_lock.
...
- Calculation phase: SV_NGA aggregates metrics and produces a numerical value for monitoring.
- This value is transmitted to CR_SYN accompanied by a unique cycle identifier.
**Persistence and Diffusion Phase**:
1. **Step 1 (Writing)**: Acquisition of coordination lock, preparation of entry in local memory, call to CR_MEM.atomic_write, then immediate release of coordination lock.
2. **Step 2 (Diffusion)**: Acquisition of global finalization lock.
Asynchronous publication via CR_HUB to subscribers (SV_CFG, CR_GSC).
3. **Confirmation Timeout**: 10 seconds.
If timeout reached: retention of last valid Global_Score, warning level alert SV_LOG, cycle marked as "partial_propagation".

**Finalization Phase**: Once diffusion is complete (with or without complete confirmations), CR_SYN updates status to "finalized", atomically copies the current entry to stable space, then releases finalization lock.
...
## CR_GSC: Global Stability Controller
**Role**: Supervises all modules to detect global oscillations, including neural modules.
- Sole arbiter of mode transitions. Receives neural activation decisions from SV_NGA with their full justification.
**Features**:
- Emergency kill-switch for unstable processes
- Aggregation of health metrics (latency, errors, resources)
- Triggering alerts via SV_LOG and CR_HUB
- Coordination of recovery strategies with SV_NIE and SV_NGA
- Provision of stability metrics to CR_HUB for global diffusion
- Automatic triggering of MAINTENANCE modes (unchanged criteria)
**SV_NGA Alerts Monitoring**: In case of persistent degradation (neural activation refused for more than 15 minutes with reason "VETO_VALIDATION" or "VETO_STABILITY"), CR_GSC can trigger a transition to MAINTENANCE_STABILIZATION mode for in-depth investigation.
...
**Standardized Interfaces**:
- `check_system_stability()`: Evaluates global stability using SV_EVF metrics to filter external OS noise.
- Provides Stability_Index (0-100), operational score used for mode transition decisions.
- This score is calculated independently of the Global_Monitoring_Score produced by SV_NGA.
- Return dict with stable: bool, metrics: dict (including cpu_elia, ram_elia_gb, noise_ratio from SV_EVF), noise_detected: bool.
- `trigger_emergency_stop(reason)`: Emergency stop.
- Contract: reason: str.
- `get_recovery_strategy()`: Recommended recovery strategy.
- Return: str or dict.
- `deposit_stability_metrics()`: Deposits stability metrics in shared cache accessible by CR_HUB.
...
- Transmission: Metrics sent to CR_HUB cache space via dedicated channel.
- Atomicity: Deposit performed in indivisible operation (all or nothing).
- Resilience: If CR_HUB temporarily unavailable, deposit ignored without error, new attempt next cycle.
- Guarantee: Deposited metrics immediately available for next CR_HUB read.
...
- Proper shutdown via CR_HUB if critical drift >5min.
- Coordination with SV_CFG for adjustments in MAINTENANCE mode.
...
- CR_MEM delegates ACID guarantees to the underlying SQLite engine:
...
- Management of SQLite errors and propagation to CR_SYN
- No coordination or orchestration logic
- Role of CR_SYN (orchestrator):
- Preparation of multi-key transactions in local memory
- Sequential calls to CR_MEM.atomic_write() for each key

Application rollback management if a key fails (compensatory call)
- Coordination of locks between modules
- Clarification: CR_MEM is passive in the sense that it makes no business decisions and calls no other module.
...
- Heavy maintenance operations (complete index reorganization, full vacuum) are reserved for proper shutdown phase or MAINTENANCE mode with explicit write pause, orchestrated by CR_SYN.
...
- In case of saturation (latency >100ms detected), temporary switch to read-only mode can be activated by CR_SYN to preserve critical performance.
During this mode, CR_SYN accumulates writes in a local memory buffer (maximum capacity 100 operations or 10MB).
Once CR_MEM returns below acceptable latency threshold (detected by 3 consecutive operations under 50ms), the buffer is emptied in batches of 20 writes grouped into unique transactions.
If the buffer reaches its maximum capacity before desaturation, new non-critical writes are temporarily rejected with warning level alert SV_LOG, while critical writes (system state, backups) switch to blocking synchronous mode with extended timeout (500ms).
**Main Functions**:
...
**Access Policy**:
- Write and read: ALWAYS via CR_SYN (security, consistency, atomicity).
- Synchronization: Orchestrated by CR_SYN via atomic locks and ACID transactions.
**Consistency Policy**: Single source of truth per type, synchronization via atomic locks orchestrated by CR_SYN.
...
- Event propagation via CR_HUB (publish/subscribe). Mandatory key prefixes for isolation: `hot_` (L1), `warm_` (L2).
...
- Cache invalidation: CR_HUB publishes `cache_invalidate` with key → CR_SYN evicts L1 → CR_MEM evicts L2.
...
2. CR_SYN reads the last N feedbacks from L2 (allowed cross read)
3. CR_SYN calculates average in local memory (no write during this calculation)
4. CR_SYN writes the `satisfaction_avg` result in L1 (derived metadata)
...
- Possible indirect influence: If a satisfaction alert triggers a system mode transition via CR_GSC, then all operations are impacted by the new mode (load reduction, temporary deactivation of non-essential modules).

**Standardized Interfaces** (contracts via Pydantic for strict validation):
- `receive_log_event(event)`: Receives event from CR_HUB.
...
- Fields: type: "feedback", user_id: str, feedback_type: enum["implicit", "explicit"], value: float (0-1, satisfaction score), context: dict (conversation_id, message_id, input_hash), timestamp: float (automatic), source: str (default "SV_UIP").
...
- Exception: If resilience_score < 40 for > 2 hours → CR_GSC triggers forced MAINTENANCE_DEGRADED mode (extreme protection).
...
- Action: Notification CR_GSC, suggestion to check host system load.
...
- New metric `end_to_end_latency`: Measurement: Timestamp entry SV_UIP → timestamp exit SV_UIP.
...
- Alert: If P95 exceeds the objective of its category for 5 consecutive minutes → notification CR_GSC with recommendation (quantization, model change, or hardware upgrade).
- New metric: `cache_eviction_rate` (evictions/minute)

**Supervision & Monitoring**:
- CPU/RAM/GPU collection via `psutil`.
- Adaptive thresholds, watchdog/restart.
**CR_GSC Watchdog**: Monitors CR_GSC presence via subscription to "system_heartbeat" topic emitted by CR_HUB.
- Checks presence and freshness of stability_metrics field in heartbeat payload.
- If field absent, null, or obsolete timestamp (difference between heartbeat receipt time and metrics timestamp greater than 15 seconds), triggers critical alert "CR_GSC_UNRESPONSIVE" with automatic restart attempt via system signal (SIGHUP or OS equivalent).
...
SV_CFG adjusts max_concurrent_requests by observing three independent operational signals:

1. Stability_Index provided by CR_GSC (system stability score 0-100)
2. CPU/RAM pressure measured by SV_LOG (raw metrics)
3. Rejection rate observed by SV_UIP (percentage on sliding window)

These three signals are distinct from the Global_Monitoring_Score calculated by SV_NGA, which remains a passive metric.
...
- Propagation of changes via CR_HUB.
...
These values apply to the SV_NIE.inference() interface and are propagated via SV_CFG at startup.
...
1. SV_EVF runs a light synthetic load (equivalent to 3-5 simulated user requests per minute)
...
If deviation > 15% between old and new threshold, notification to CR_GSC for validation before application.
**Default Profiles**:
...
- Alert if noise_ratio >0.5 for 5 minutes (notification CR_GSC).
- Background periodic report every 10 minutes.

---

## SV_ICF: Input Classification & Filtering
...
- Relevance scoring (0-1) based on CR_MEM context.
...
**Neural Validation Activation**: activated if `neural_processing = True` (flag determined by SV_NGA via hierarchical evaluation).
...
- SV_HVE accesses Global_Monitoring_Score only for internal analytical and non-decisional needs.
...
## SV_NGA: Neural Gating Authority
...
If validation score < 60, stability < 40, or excessive OS noise (SV_EVF invalidation), activation blocked.
...
- VETO_OS_NOISE: Cycle invalidated by SV_EVF (Level 1)
...
1. SV_UIP collects feedback → SV_LOG (storage + passive monitoring)
2. SV_LOG calculates aggregated satisfaction metrics (sliding average, trend)
3. If satisfaction < 0.4 for 10 consecutive cycles → SV_LOG emits "low_user_satisfaction" alert
4. CR_GSC, receiving this alert, may decide on a transition to MAINTENANCE_STABILIZATION for investigation

- Important clarification:
- Feedback does NOT enter the neural eligibility calculation performed by SV_NGA.evaluate_neural_eligibility().
- However, if low feedback triggers a global system mode change (CR_GSC decision), then the new mode indirectly impacts all operations, including neural activation.
- This influence is indirect, asynchronous, and passes through human or supervised arbitration (CR_GSC).
...
## SV_UIP: User Interaction Point
**Role**: User entry/exit point, distinct from EX_IGC (which manages external APIs).
- Integrates with core/ via CR_HUB for message routing and SV_ICF for input classification.
**Features**:
**Conversational Only**:
- Web/mobile chat.
- Vocal interface.
...
**Separate Administrator Interface** (via SV_UIP in admin mode):
...
- Limitation of active simultaneous requests number: configurable threshold via SV_CFG (default 5).
- System stability check via CR_GSC before acceptance.
...
1. Each accepted request records its `request_id` and timestamp in CR_SYN
2. During processing, SV_UIP sends a heartbeat every 5 seconds to CR_SYN to signal that the request is still active

**Heartbeat Implementation**: Separate non-blocking thread launched via recursive `threading.Timer`.
If sending fails (CR_SYN temporarily unavailable), retry attempt after 2 seconds (maximum 3 attempts).
If 3 attempts fail, heartbeat marked as "failed". The admission slot is released automatically. The request is abandoned gracefully — processing cannot continue without CR_SYN.
The 30-second timeout on CR_SYN side remains unchanged.

3. CR_SYN monitors heartbeats: if no ping received for 30 seconds for a given `request_id`, slot automatically released
4. SV_LOG receives "orphan_request_detected" alert with details for post-mortem investigation

- Guarantee:
- In case of SV_UIP crash (OOM, SIGKILL, network loss), slot released in maximum 30 seconds, preventing permanent system blockage.
- This mechanism is transparent for business code (no explicit call required, managed by internal SV_UIP decorator).
**Parameters**:
- `max_concurrent_requests`: 5 by default (adjustable via SV_CFG sv_uip section).
- `rejection_cooldown`: 30 seconds (suggested time before user retry).
- `critical_mode_threshold`: Compromised system stability (CR_GSC) activates preventive rejections.
**Activation Conditions**: Always active in INTERACTIVE mode.

**Triggers**:
- Each user message → SV_ICF via CR_HUB.
...
## SV_NIE: Neural Inference Engine
...
- Interaction with CR_MEM via CR_SYN only.
...
If timeout is None, the value is retrieved from SV_CFG according to the active hardware profile (low: 12000ms, medium: 8000ms, high: 5000ms).
- Returns a string or dictionary with confidence score if applicable.
**Timeout Management by Caller**:
Timeout is managed by the calling module (SV_DLG), not by SV_NIE itself.
Mechanism:
- SV_DLG launches SV_NIE call in a dedicated thread via ThreadPoolExecutor
- SV_DLG simultaneously starts a timer corresponding to the configured timeout (timeout configured according to SV_CFG profile)
- If timer expires before SV_NIE return:
- SV_DLG cancels thread via future.cancel()
- SV_DLG generates a structured TimeoutException
- SV_DLG transmits exception to SV_LOG with full context
- SV_DLG immediately switches to fallback symbolic processing
- If SV_NIE returns before expiration: normal processing

Critical Guarantee: A complete SV_NIE blockage (GPU freeze, internal deadlock) never blocks SV_DLG.
...
Content of structured exception transmitted to SV_LOG:
...
- Probable deduced reason by SV_DLG (GPU saturation, insufficient memory, model not loaded)
...
**Activation Conditions**: Neural processing is activated if `neural_processing = True` (flag calculated by SV_NGA via hierarchical cascade).
- Hardware profile conditions (RAM, CPU) are checked before each inference.

**Relations**:
- Called from SV_DLG (generation).
- Supports SV_LRN (fine-tuning, LoRA).
- Supervision by SV_LOG (response time, resources).

---

## SV_LRN: Targeted Learning
...
- Before starting any session, SV_LRN performs a preliminary check of the provided dataset characteristics.
...
- In case of detected non-compliance, session immediately rejected with explicit error message (logged in SV_LOG) indicating applicable limit and observed value.
...
- SV_LOG detected cases to process.
...
## EX_EGC: Egress Connector (Outbound)
**Features**:
...
- Timeout: 10s per request (configurable via SV_CFG).
...
- Alerts: Notification SV_LOG if all APIs of a type fail.
**Standardized Interfaces** (contracts via Pydantic for strict validation):
...
## EX_IGC: Ingress Connector (Inbound)
**Role**: External REST API for third-party integrations and incoming webhooks (distinct from SV_UIP).
**Features**:
...
- Quality metrics monitoring including SV_NGA scores.
**Standardized Interfaces** (contracts via Pydantic for strict validation):
...
**Triggers**: Activation if `debug_mode = True` or SV_LRN (learning processes).
...
Used by SV_CFG to identify distribution (Ubuntu, Debian, Alpine...), kernel version (cgroups v1/v2 detection), CPU architecture (x86_64, ARM64) and determine containerization via system inspection.
SV_EVF uses this data to interpret psutil metrics according to isolation context.

### Data & Storage

- `sqlite3`, `sqlcipher`, `sqlalchemy`: Databases (embedded SQLite, SQLCipher encryption, SQLAlchemy ORM) for CR_MEM persistence.
- `orjson`: Ultra-fast JSON secure serialization with Pydantic schemas validation for inter-module messages.
- `whoosh`: Full-text textual search for conversations indexing and CR_MEM context search.

### NLP & AI

- `spacy`: Natural language processing (tokenization, NER, POS tagging) for symbolic validation SV_HVE.
- `scikit-learn`: Machine learning (classification, clustering) for SV_ICF inputs scoring and SV_LOG patterns detection.
- `sentence-transformers`: Semantic embeddings for text vector representation and similarity search CR_MEM.
- `transformers`: Hugging Face neural models (Llama 3.2 3B, Phi-3 Mini, DistilBERT, DeBERTa-v3-small) for SV_NIE inference.
...
- `pyaudio`, `librosa`: Audio (capture, signal processing, MFCC features extraction) for vocal inputs SV_ICF.
...
- `opencv-python`: Vision (camera capture, image processing, objects detection) for visual inputs SV_ICF.
- `pyttsx3`: Vocal synthesis (TTS) for vocal outputs SV_UIP.

### Web & Network

- `requests`: HTTP client with retry, timeout, and sessions management for EX_EGC API calls.
- `fastapi`, `streamlit`: Web interfaces (FastAPI REST API for EX_IGC, Streamlit dashboard for admin SV_UIP).
- `beautifulsoup4`: HTML/XML parsing for web content extraction EX_EGC.
- `scrapy`: Ethical web scraping with robots.txt respect for structured data collection.

### Testing & Monitoring

- `pytest`, `pytest-asyncio`: Unit, integration, and asynchronous tests for modules validation SV_TSR.
- `cerberus`, `jsonschema`: Configuration and messages schemas validation for CR_SEC and SV_CFG.
...
- SV_EVF validation (acceptable OS noise)
...
- SV_EVF.is_cycle_valid() function returns True (OS noise below profile threshold)

Invalidated cycles (either by lack of requests or excessive OS noise) retain the Global_Monitoring_Score of the previous cycle and are recorded in SV_LOG for later analysis, but do not enter trend calculations.
...
1. CR_SEC: Cryptographic keys generation (175ms)
2. CR_HUB: Broker connection + routing tables loading (350ms)
3. CR_MEM: SQLite WAL mounting + integrity check (1400ms)
4. CR_SYN: CR_MEM accessibility check via test read attempts (maximum 15s with exponential backoff).
If failure: immediate shutdown error code "STORAGE_UNREACHABLE".
5. SV_LOG: Minimal memory buffer initialization (logging active immediately)
6. Operational modules: SV_CFG, SV_EVF, CR_GSC, SV_UIP (total ~1000ms)
**Pre-flight Check**: Imperative validation of CR_MEM physical accessibility (disk/socket mount) before effective CR_SYN launch.
...
Routing activation via CR_HUB.
Detection and application of environment profile (Linux Docker, etc.).
**Operational Stabilization (variable duration, see CR_SYN - Adaptive warm-up for detailed criteria, typically 5-20 minutes)**: Observation period (formerly "warm-up") allowing the system to validate its stability before full activation.
...
3. **No Active Alerts**: No critical or warning alert active in SV_LOG at restoration time.
...
- SV_NGA immediately evaluates neural eligibility via evaluate_neural_eligibility()
...
- User inputs collection (SV_ICF).
- Filtering and normalization.
- Context extraction from CR_MEM via CR_SYN.
- Input validation via SV_HVE (symbolic only during stabilization).
- Systematic admission control via SV_UIP before processing.
...
1. **Cycle Validation via SV_EVF**: Query SV_EVF.is_cycle_valid() once per cycle before any costly operation.
...
- Invalidation logging in SV_LOG with CycleInvalidationLogEntry (noise_ratio, reason, timestamp).
- Direct passage to Phase 3 (OUTPUT) with degraded response or hold.

3. **If Valid Cycle (Acceptable OS Noise)**:

**Neural Eligibility Evaluation via SV_NGA.evaluate_neural_eligibility()**: Hierarchical analysis (Security/Vetos, Quality, Context) to determine `neural_processing` flag.
- Request analysis via SV_ICF (classification, routing).
- Response generation via SV_DLG.
- Optional calls to SV_NIE if `neural_processing = True` (validated by SV_NGA) and resources available.
- Output validation via SV_HVE (symbolic + neural if activated by flag).
**Global_Monitoring_Score Calculation via SV_NGA.compute_global_score()**: Purely informative calculation for monitoring and dashboards, executed after operational decisions.
- Systematic logging of OS context for any neural operation (cpu_elia, ram_elia_gb, noise_ratio in NeuralLogEntry.os_context).

4. **Finalization**:

- Persistence of Global_Monitoring_Score in CR_MEM via CR_SYN if valid cycle.
- Acquisition of global finalization lock to guarantee mutual exclusion of cycles during this phase.
- Event propagation via CR_HUB.
**Guarantee**: Validation ALWAYS before calculations to avoid metrics pollution by external OS noise.

### Phase 3: Output (OUTPUT)

- Final output validation (SV_HVE).
- Delivery to SV_UIP.
- Conversation storage in CR_MEM (working.conversations space, L2 update).
- Feedback collection via SV_UIP:
- Feedback published to CR_HUB "user_feedback" topic.
- CR_HUB routes only to SV_LOG (storage and alert analysis).
- Feedback does NOT influence next cycle neural calculation (decoupling).
- Update user_context in CR_MEM (satisfaction_avg, interaction_count, L1 update).
- Update Global_Score_previous (stable monitoring version) for next cycle via CR_SYN (atomic write).
- Recording of final OS metrics in SV_LOG for performance correlation.
- Admission slot release in SV_UIP.
- Update of resilience statistics in CR_SYN.

### Parallel Phase: Maintenance (BACKGROUND)

- Targeted learning (SV_LRN) if conditions met (monitoring `neural_processing` flag).
- Parametric optimization (SV_CFG) based on operational signals convergence.
- Continuous monitoring via SV_LOG and CR_GSC.
- Archiving old data (CR_MEM).
...
1. Shutdown signal reception via CR_HUB.
2. CR_GSC coordinates progressive shutdown.
3. Completion of in-progress requests.
4. Complete state backup via CR_MEM.
5. Closure of external connections (EX_EGC, EX_IGC).
6. Neural models unloading (SV_NIE).
7. Final logs export (SV_LOG).
8. CR_HUB shutdown (routing stopped, no more messages transit).
9. CR_SYN shutdown (final state backup after routing shutdown, guarantees consistency).
10. CR_SEC shutdown last (cryptographic sessions closure after all modules).
